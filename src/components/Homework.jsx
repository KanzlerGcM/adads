...
function PartArrange({ parte, isPt }) {
  const [revealed, setRevealed] = useState({});
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {parte.tip && (
        <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 8, padding: "8px 14px", fontSize: 12, color: "#1e3a5f" }}>
          💡 {isPt ? parte.tip.pt : parte.tip.en}
        </div>
      )}
      {parte.items.map((item, i) => (
        <div key={i} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, flexWrap: "wrap" }}>
            <div>
              <span style={{ fontWeight: 800, color: "var(--primary)", marginRight: 8 }}>{i + 1}.</span>
              <div style={{ display: "inline-flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                {item.words.map((w, wi) => (
                  <span key={wi} style={{ background: "#eef2ff", color: "#3730a3", border: "1.5px solid #c7d2fe", borderRadius: 7, padding: "4px 10px", fontWeight: 700, fontSize: 13 }}>{w}</span>
                ))}
              </div>
              {isPt && item.pt && <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 6 }}>{item.pt}</div>}
              {item.note && <span style={{ background: "#fef3c7", color: "#92400e", fontSize: 11, fontWeight: 600, padding: "1px 8px", borderRadius: 999, marginTop: 4, display: "inline-block" }}>📌 {item.note}</span>}
            </div>
            <button
              onClick={() => setRevealed(r => ({ ...r, [i]: !r[i] }))}
              style={{ padding: "5px 12px", borderRadius: 999, border: "1.5px solid var(--primary)", background: revealed[i] ? "var(--primary)" : "transparent", color: revealed[i] ? "#fff" : "var(--primary)", fontWeight: 700, fontSize: 12, cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}
            >
              {revealed[i] ? "✕ Ocultar" : "✅ Ver resposta"}
            </button>
          </div>
          {revealed[i] && (
            <div style={{ marginTop: 10, background: "#d1fae5", border: "1px solid #6ee7b7", borderRadius: 8, padding: "8px 14px", fontSize: 14, fontWeight: 700, color: "#065f46" }}>
              ✅ {item.answer}
            </div>
          )}
        </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      {parte.items.map((item, i) => {
        ...
        const allValid = correct === -1;
        return (
          <div key={i} style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, padding: "14px 16px" }}>
            <div style={{ fontWeight: 700, fontSize: 14, color: "var(--text)", marginBottom: 4 }}>
              <span style={{ color: "var(--primary)", marginRight: 6 }}>{i + 1}.</span>{item.question}
            </div>
            {isPt && item.pt && <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 10 }}>{item.pt}</div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {item.options.map((opt, oi) => {
                const isCorrect = allValid || oi === correct;
                const isWrong = picked !== undefined && picked === oi && !isCorrect;
                const showResult = picked !== undefined;
                let bg = "var(--bg-card)", border = "var(--border)", color = "var(--text)";
                if (showResult && isCorrect) { bg = "#d1fae5"; border = "#6ee7b7"; color = "#065f46"; }
                if (showResult && isWrong)   { bg = "#fee2e2"; border = "#fca5a5"; color = "#991b1b"; }
                return (
                  <button
                    key={oi}
                    disabled={picked !== undefined}
                    onClick={() => setSelected(s => ({ ...s, [i]: oi }))}
                    style={{ display: "flex", alignItems: "center", gap: 10, background: bg, border: `1.5px solid ${border}`, borderRadius: 8, padding: "8px 14px", cursor: picked !== undefined ? "default" : "pointer", fontFamily: "inherit", textAlign: "left", transition: "0.15s" }}
                  >
                    <span style={{ fontWeight: 800, fontSize: 12, color: "var(--primary)", minWidth: 18 }}>{letters[oi]}.</span>
                    <span style={{ fontWeight: 600, fontSize: 14, color }}>{opt}</span>
                    {showResult && isCorrect && <span style={{ marginLeft: "auto", fontSize: 14 }}>✅</span>}
                    {showResult && isWrong   && <span style={{ marginLeft: "auto", fontSize: 14 }}>❌</span>}
                  </button>
                );
              })}
            </div>
            {item.note && picked !== undefined && (
              <div style={{ marginTop: 8, background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 7, padding: "7px 12px", fontSize: 12, color: "#1e3a5f" }}>
                💡 {item.note}
              </div>
            )}
          </div>
...
function HwPart({ parte, isPt }) {
  return (
    <div className="card" style={{ marginBottom: 20 }}>
      ...
      {parte.type === "arrange"  && <PartArrange  parte={parte} isPt={isPt} />}
      {parte.type === "mc"       && <PartMC       parte={parte} isPt={isPt} />}
    </div>
  );
}
